...
class MyAgentSupport(FinanceSimulationAgent):
    def initialize(self):
        self.history_dirs = {}
    ...
    def get_next_price(price, tick_size, direction):
    ...
    def respond(self, state):
        ...
        if validator_hotkey == '5EWwdZB7qCCMaAso5Mzcks4UUcPxKYvpAj32t5Mg1v6HSxoF':
            self.support_time = not self.support_time
            print(f"Support time toggled to {self.support_time}")
        for book_id, book in state.books.items():
            ...
            buy_or_sell = random.choice([OrderDirection.BUY, OrderDirection.SELL])
